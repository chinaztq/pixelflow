...
export async function POST(
  ...
  if (!session?.user)
    return buildApiError("UNAUTHORIZED", "请先登录", 401);
  ...
  if (!brief)
    return buildApiError("NOT_FOUND", "需求单不存在", 404);
  ...
  if (!canUpload)
    return buildApiError("FORBIDDEN", "无权限", 403);
  ...
  const basedOnVersionId = formData.get("basedOnVersionId") as
    | string
    | null;

  if (files.length === 0)
    return buildApiError("VALIDATION_ERROR", "请上传文件", 400);
  ...
  try {
    ...
    // Clean up: delete version (cascades to images) and saved files
    await prisma.version
      .delete({ where: { id: version.id } })
      .catch(() => {});
    ...
    return buildApiError(
      "PROCESSING_ERROR",
      `图片处理失败: ${err instanceof Error ? err.message : "未知错误"}`,
      500
    );
  }
